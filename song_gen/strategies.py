...
class SunoSongGeneratorStrategy(SongGeneratorStrategy):
    ...
    def _extract_task_id(self, payload: Any) -> str | None:
        """Extract task ID from common Suno response envelopes."""
    ...
    def _extract_error_message(self, payload: Any) -> str:
        """Extract human-readable error message from API responses."""
        if isinstance(payload, dict):
    ...
    def _extract_audio_url(self, payload: Any) -> str | None:
        """Extract audio URL from common and nested provider response shapes."""
        if isinstance(payload, dict):
            for key in (
                "audio_url",
                "audioUrl",
            ...
            # Recurse through nested structures where audio can be buried.
